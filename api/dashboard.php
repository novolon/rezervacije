...
try {
    if ($session['role'] === 'admin' && !$rest_id) {
        // Admin brez filtra: vse aktivne restavracije
        $stmt = $pdo->prepare("
        ...
        // Admin z izbranim filtrom ali user: samo določena restavracija
        $rid = ($session['role'] === 'admin') ? $rest_id : (int)$session['restaurant_id'];
        $stmt = $pdo->prepare("
