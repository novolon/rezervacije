...
<main class="rz-main">
    <div class="rz-topbar">
        <div>
            <h1 class="rz-h1">Profil</h1>
            <p class="rz-top-sub">Osebni podatki, bančne informacije in varnost</p>
        </div>
    </div>
    ...
    <div class="rz-card" style="margin-bottom:16px">
        <div class="rz-card-head">
            <h2 class="rz-card-title" style="font-size:15px">Osebni podatki & bančne informacije</h2>
        </div>
        ...
        <form id="profile-form" class="rz-form">
            <div class="rz-form-2">
                ...
                <div class="rz-field">
                    <label class="rz-field-label">Email</label>
                ...
                <div class="rz-field">
                    ...
                    <select id="legal_form" name="legal_form" class="rz-input">
                        ...
                        <option value="foreign" <?= ($aff['legal_form'] ?? '') === 'foreign' ? 'selected' : '' ?>>Tujina</option>
                    </select>
                </div>
                <div class="rz-field">
                    <label class="rz-field-label">Davčna številka</label>
                ...
                <div class="rz-field" style="grid-column:1/-1">
                    <label class="rz-field-label">Naslov</label>
            ...
            <div>
                <button type="submit" class="rz-btn rz-btn-primary">Shrani spremembe</button>
            </div>
        </form>
    </div>
    ...
    <div class="rz-card" style="margin-bottom:16px">
        <div class="rz-card-head">
            <h2 class="rz-card-title" style="font-size:15px">Sprememba gesla</h2>
        </div>
        <div id="pw-msg"></div>
        <form id="pw-form" class="rz-form">
            <div class="rz-field">
                <label class="rz-field-label">Trenutno geslo</label>
            ...
            <div>
                <button type="submit" class="rz-btn">Spremeni geslo</button>
            </div>
        </form>
    </div>
    ...
    <div class="rz-card">
        <div class="rz-card-head">
            <h2 class="rz-card-title" style="font-size:15px">Podatki o računu</h2>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;font-size:13px">
            <div>
                <div style="font-size:10px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--ink-mute);margin-bottom:6px">Referenčna koda</div>
            ...
            <div>
                <div style="font-size:10px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--ink-mute);margin-bottom:6px">Status</div>
                <?php
                $st = $aff['status'] ?? '';
                $stMap = [
                    'active'    => '<span class="rz-chip rz-chip-confirmed">Aktiven</span>',
                    'pending'   => '<span class="rz-chip" style="background:color-mix(in oklab,var(--warning) 15%,transparent);color:var(--warning);border-color:transparent">V pregledu</span>',
                    'suspended' => '<span class="rz-chip" style="background:color-mix(in oklab,var(--danger) 10%,transparent);color:var(--danger);border-color:transparent">Suspendiran</span>',
                ];
                echo $stMap[$st] ?? htmlspecialchars($st, ENT_QUOTES);
                ?>
            </div>
            <div>
                <div style="font-size:10px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--ink-mute);margin-bottom:6px">Datum registracije</div>
                <span style="font-family:var(--font-mono)"><?= htmlspecialchars(substr($aff['created_at'] ?? '', 0, 10), ENT_QUOTES) ?></span>
            </div>
            <div>
                <div style="font-size:10px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:var(--ink-mute);margin-bottom:6px">Email potrjen</div>
                <?= $aff['email_verified_at']
                    ? '<span class="rz-chip rz-chip-confirmed">Da</span>'
                    : '<span class="rz-chip rz-chip-mute">Ne</span>' ?>
            </div>
        </div>
    </div>
</main>
...
document.getElementById('profile-form').addEventListener('submit', async e => {
    ...
    try {
        ...
        showMsg('profile-msg', d.success ? 'Podatki shranjeni.' : (d.error ?? 'Napaka.'), d.success);
    } catch(err) {
        showMsg('profile-msg', 'Napaka pri shranjevanju.', false);
    }
});
...
document.getElementById('pw-form').addEventListener('submit', async e => {
    ...
    if (np !== np2) { showMsg('pw-msg', 'Gesli se ne ujemata.', false); return; }
    const body = {
    ...
    try {
        const r = await fetch(BASE + '/api/affiliate.php', { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(body) });
        const d = await r.json();
        showMsg('pw-msg', d.success ? 'Geslo posodobljeno.' : (d.error ?? 'Napaka.'), d.success);
        if (d.success) e.target.reset();
    } catch(err) {
        showMsg('pw-msg', 'Napaka pri shranjevanju.', false);
    }
});
</script>
</body>
